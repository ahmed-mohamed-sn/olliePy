...
def check_attributes_type(title, output_directory, subtitle, report_folder_name, encryption_secret,
                          generate_encryption_secret):
    if type(title) is not str:
        raise TypeError(f'provided title is not valid. title has to be str')
    if type(output_directory) is not str:
        raise TypeError(f'provided output_directory is not valid. output_directory has to be str')
    ...
    if subtitle is not None and type(subtitle) is not str:
        raise TypeError(f'provided subtitle is not valid. subtitle has to be str')
    if report_folder_name is not None and type(report_folder_name) is not str:
        raise TypeError(f'provided report_folder_name is not valid. report_folder_name has to be str')
    if encryption_secret is not None and type(encryption_secret) is not str:
        raise TypeError(f'provided encryption_secret is not valid. encryption_secret has to be str')
    ...
    if type(generate_encryption_secret) is not bool:
        raise TypeError(f'provided generate_encryption_secret is not valid. encryption_secret has to be bool')
...
class Report:
    ...
    def __init__(self,
        ...
        check_attributes_type(title, output_directory, subtitle, report_folder_name, encryption_secret,
                              generate_encryption_secret)
